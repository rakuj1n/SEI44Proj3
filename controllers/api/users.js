...
async function updateFriend(req, res) {
  ...
  try {
    ...
    if (userToAdd._id.toString() === req.params.userId)
      throw new Error("Cannot add self.");
    const account = await Account.findOne({ user: userId });
    if (account.following.includes(userToAdd._id))
      throw new Error("Already added.");
    await Account.findOneAndUpdate(
      { user: userId },
      { $addToSet: { following: userToAdd._id } }
    );
    console.log("saved");
    res.status(200).json(account);
  } catch (err) {
  ...
  try {
    ...
    const array = await Account.aggregate([
      ...
      { $unwind: { path: "$moviesRecommended" } },
      {
        $group: {
          _id: null,
          moviesRecommended: { $addToSet: "$moviesRecommended" },
        },
      },
    ]);
...
async function updateMoviesRecommended(req, res) {
  ...
  try {
    ...
    if (account.moviesRecommended.includes(req.body.movieId))
      throw new Error("Already added.");
    await Account.findOneAndUpdate(
      { user: userId },
      { $addToSet: { moviesRecommended: req.body.movieId } }
    );
    res.status(200).json(account);
  } catch (err) {
    res.status(400).json(err);
...
async function updateMoviesWatched(req, res) {
  const userId = req.params.userId;
  try {
    const account = await Account.findOne({ user: userId });
    if (!account) throw new Error("Account not found.");
    if (account.watchHistory.includes(req.body.movieId))
      throw new Error("Already added.");
    await Account.findOneAndUpdate(
      { user: userId },
      { $addToSet: { watchHistory: req.body.movieId } }
    );
    res.status(200).json(account);
  } catch (err) {
    res.status(400).json(err);
