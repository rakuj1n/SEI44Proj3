...
export default function MoviePlayAndCommentPage({ user }) {
  ...
  const handleSubmit = async (event) => {
    ...
    if (rating > 2) {
      ...
      //Submit recommended
      try {
        await sendRequest(`/api/users/${user._id}/movies-recommended`,'PUT', {"movieId":state.state.movieDetails._id})
      } catch (err) {
        console.log(err)
      }
    }
    ...
    if (comment.length !== 0) {
      ...
      try {
        await sendRequest(`/api/movies/comments/${user._id}/${state.state.movieDetails._id}`,'PUT', {"comment":comment})
      } catch (err) {
        console.log(err)
      }
