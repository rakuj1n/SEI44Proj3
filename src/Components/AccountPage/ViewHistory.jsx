...
export default function ViewHistory({ account, user }) {
  ...
  const watchedList = account?.watchHistory.map((item) => {
    const comment1 = item.comments.filter(
      (item) => item.userId === account.user._id
    );
        ...
        {isUser && (
          <div className="commentsectionitem commentwedit">
            <Link
              state={{
                item,
                comment: comment1[0]?.comment,
                rating: comment1[0]?.rating,
              }}
              className="editcomment"
              to={`/users/${account?.user._id}/${item?._id}/editcomment`}
            >
              Edit Review
            </Link>
            <div>
              ...
              <p style={{ margin: "0", marginBottom: "0" }}>
                <strong>
                  {comment1[0]?.rating == 0 ? "" : comment1[0]?.rating}
                </strong>{" "}
                <StarOutlined />
              </p>
            </div>
            {comment1[0]?.comment.length > 0 ? (
              <p>
                {account?.user.name}: <em>"{comment1[0]?.comment}"</em>
              </p>
            ) : (
              <p>
                <em>No review yet.</em>
