...
export default function MovieRecoList({
  currSelectedFollowingAccount,
  currSelectedFollowing,
  account,
  allFollowingMovieRecoList,
}) {
  console.log("currSelectedFollowing", currSelectedFollowing);

  const movieRecoList = currSelectedFollowingAccount?.moviesRecommended.map(
    (item) => {
      return (
        <div
          className="movieitem"
        ...
        >
          ...
          {item.comments.find(
            (item) => item.userId == currSelectedFollowing
          ) && (
            <div className="commentsectionitem">
              <div>
                <img
                  className="profilepic"
                  src={`${currSelectedFollowingAccount?.user.picture}`}
                />
                <p style={{ margin: "0" }}>
                  {
                    item.comments.find(
              ...
              <p>
                {
                  account?.following.find(
                    (item) => item?._id === currSelectedFollowing
                  )?.name
                }
                :{" "}
                <em>
    ...
    <div className="followingrecommendationscontainer">
      ...
      {currSelectedFollowing && (
        <div
          ...
          className="movielist"
        >
          {movieRecoList?.sort((a, b) => b.key - a.key)}
        </div>
      )}
    </div>
  );
